...
class Bell extends Component {
 ...
 ringBell = async () => {
    ...
    try {
      ...
      await certificate.methods.ringBell().send({
        from: accounts[0],
        value: web3.utils.toWei(this.state.giftAmount, 'ether')
      });

      this.setState({ successMessage: `You've successfully given ${this.state.giftAmount} ether to this certificate owner` });
    } catch (err) {
    ...
    return (
      <span className='bell fa fa-bell'>
        <Modal id='Ring-Bell-Modal' trigger={
          ...
          <Modal.Header >Ring Reward Bells</Modal.Header>
          <Modal.Content>
            ...
            <Modal.Description id='Modal-Description'>
              ...
              <p>You must really like this certificate owner :) </p>
